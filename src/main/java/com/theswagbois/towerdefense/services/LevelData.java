...
public class LevelData {
    ...
    public static void loadLevelData() {
        ...
        for (int i = 1; i <= levelCount; i++) {
            HashMap<String, Object> levelData = getAssetLoader().loadJSON("levels/level" + i + ".json", HashMap.class).get();
            int pointsCount = levelData.size();
            Point2D spawnPoint = new Point2D(0, 0);
            List<Point2D> waypoints = new ArrayList<Point2D>();
            for (int j = 0; j < pointsCount; j++) {
                HashMap<String, Object> pointData = (HashMap<String, Object>) levelData.get(String.valueOf(j));
